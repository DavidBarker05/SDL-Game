...
bool GameRenderer::Init()
{
	...
	m_pGPUDevice = SDL_CreateGPUDevice(SDL_GPU_SHADERFORMAT_INVALID, FALSE, NULL); // Forces SDL to use discrete GPU
	m_pWindow = SDL_CreateWindow(DEFAULT_WINDOW_TITLE, DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT, SDL_WINDOW_HIGH_PIXEL_DENSITY);
	m_pRenderer = SDL_CreateGPURenderer(m_pGPUDevice, m_pWindow);
...
void GameRenderer::Destroy()
{
	...
	SDL_DestroyGPUDevice(m_pGPUDevice);
